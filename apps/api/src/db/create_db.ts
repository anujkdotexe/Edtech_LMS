...
async function main() {
  ...
  try {
    ...
    if (res.rowCount === 0) {
      console.log("[INFO] Creating database 'lms_gamified'...");
      await client.query("CREATE DATABASE lms_gamified");
      console.log("[OK] Database 'lms_gamified' created successfully!");
    } else {
      console.log("[OK] Database 'lms_gamified' already exists.");
    }
  } catch (error) {
    console.error('[ERROR] Error ensuring database exists:', error);
    process.exit(1);
  } finally {
    await client.end();
  }
